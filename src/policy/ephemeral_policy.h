 ...
 *  Sets out_child_wtxid on failure when provided.
 *  @returns a validation result with `IsValid() == true` if all dust is properly spent.
 */
[[nodiscard]] TxValidationState CheckEphemeralSpends(const Package& package, CFeeRate dust_relay_rate, const CTxMemPool& tx_pool, Wtxid* out_child_wtxid = nullptr);
